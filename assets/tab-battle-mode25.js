...
const BATTLE_STAT_KEYS=["atk","def","satk","sdef","spd"];
const BATTLE_STAT_LABELS={atk:"ATK",def:"DEF",satk:"SPATK",sdef:"SDEF",spd:"SPD"};
const cleanBattleNumber=value=>Math.max(0,Math.floor(Number(value)||0));
...
function BattleMode(){
  ...
  const[hpAdjust,setHpAdjust]=React.useState({left:1,right:1});
  ...
  const hp=pokemon=>{
    ...
    const max=Math.max(1,Math.floor(Number(pokemon.battleMaxHp)||calculatePokemonMaxHP(pokemon)));
    return{current:Math.max(0,max-(pokemon.currentDamage||0)),max};
  };
  const stats=pokemon=>{
    if(!pokemon)return{};
    const actual=getActualStats(pokemon),overrides=pokemon.battleStatOverrides||{};
    return BATTLE_STAT_KEYS.reduce((result,key)=>({...result,[key]:overrides[key]===undefined||overrides[key]===""?actual[key]:cleanBattleNumber(overrides[key])}),{...actual});
  };
  ...
  const updateDamage=(trainerId,pokemonId,amount)=>updatePokemon(trainerId,pokemonId,pokemon=>{const max=hp(pokemon).max,next=Math.min(max,Math.max(0,(pokemon.currentDamage||0)+amount));return{...pokemon,currentDamage:next,battleAffliction:next>=max&&["burned","cursed","poisoned","toxified"].includes(pokemon.battleAffliction?.type)?null:pokemon.battleAffliction}});
  const restore=(trainerId,pokemonId)=>updatePokemon(trainerId,pokemonId,pokemon=>({...pokemon,currentDamage:0}));
  const adjustHp=(trainerId,pokemon,amount)=>{const before=hp(pokemon).current,max=hp(pokemon).max,after=Math.max(0,Math.min(max,before+amount)),actual=after-before;if(!actual)return;updateDamage(trainerId,pokemon.id,-actual);statusLog(pokemon,`Manual HP: ${actual>0?"+":""}${actual} (${after}/${max} HP).`)};
  const setBattleMaxHp=(trainerId,pokemon,value)=>{const currentHp=hp(pokemon).current,nextMax=Math.max(1,Math.floor(Number(value)||1)),nextCurrent=Math.min(currentHp,nextMax);updatePokemon(trainerId,pokemon.id,current=>({...current,battleMaxHp:nextMax,currentDamage:nextMax-nextCurrent}))};
  const setBattleStatOverride=(trainerId,pokemon,key,value)=>updatePokemon(trainerId,pokemon.id,current=>({...current,battleStatOverrides:{...(current.battleStatOverrides||{}),[key]:cleanBattleNumber(value)}}));
  const resetBattleOverrides=(trainerId,pokemon)=>{const currentHp=hp(pokemon).current,baseMax=calculatePokemonMaxHP(pokemon),nextCurrent=Math.min(currentHp,baseMax);updatePokemon(trainerId,pokemon.id,current=>{const{battleMaxHp,battleStatOverrides,...rest}=current;return{...rest,currentDamage:baseMax-nextCurrent}});statusLog(pokemon,"Battle HP and stat overrides reset.")};
  ...
  const Fighter=({side,sideKey,trainer,trainerId,setTrainerId,pokemon,pokemonId,setPokemonId,opponent,opponentTrainerId})=>{
    ...
    return e.jsxs("section",{className:"battle-mode-fighter",children:[
      ...
      pokemon&&e.jsxs(React.Fragment,{children:[
        ...
        e.jsxs("div",{className:"battle-mode-hp-adjust",children:[e.jsx("span",{children:"Manual HP"}),e.jsx("button",{type:"button","aria-label":`Subtract ${hpAdjust[sideKey]} HP from ${displayName(pokemon)}`,title:"Subtract HP",onClick:()=>adjustHp(trainer.id,pokemon,-hpAdjust[sideKey]),children:"-"}),e.jsx("input",{type:"number",min:1,max:health.max||999,value:hpAdjust[sideKey],onChange:event=>setHpAdjust(current=>({...current,[sideKey]:Math.max(1,Number(event.target.value)||1)})),"aria-label":`${side} HP adjustment amount`}),e.jsx("button",{type:"button","aria-label":`Add ${hpAdjust[sideKey]} HP to ${displayName(pokemon)}`,title:"Add HP",onClick:()=>adjustHp(trainer.id,pokemon,hpAdjust[sideKey]),children:"+"})]}),
        e.jsxs("div",{className:"battle-mode-overrides",children:[e.jsxs("label",{children:[e.jsx("span",{children:"Battle Max HP"}),e.jsx("input",{type:"number",min:1,value:health.max,onChange:event=>setBattleMaxHp(trainer.id,pokemon,event.target.value),"aria-label":`${side} battle maximum HP`})]}),e.jsx("button",{type:"button",className:"btn btn-secondary",onClick:()=>resetBattleOverrides(trainer.id,pokemon),children:"Reset Battle Values"})]}),
        ...
        e.jsx("div",{className:"battle-mode-stats",children:BATTLE_STAT_KEYS.map(key=>e.jsxs("span",{children:[BATTLE_STAT_LABELS[key]," ",e.jsx("strong",{children:pokemonStats[key]||0})]},key))}),
        e.jsx("div",{className:"battle-mode-stat-editor",children:BATTLE_STAT_KEYS.map(key=>e.jsxs("label",{children:[e.jsx("span",{children:BATTLE_STAT_LABELS[key]}),e.jsx("input",{type:"number",min:0,value:stats(pokemon)[key]||0,onChange:event=>setBattleStatOverride(trainer.id,pokemon,key,event.target.value),"aria-label":`${side} battle ${BATTLE_STAT_LABELS[key]}`})]},key))}),
  ...
  return e.jsxs("div",{children:[
    ...
    e.jsxs("div",{className:"battle-mode-grid",children:[
      e.jsx(Fighter,{side:"Fighter 1",sideKey:"left",trainer:leftTrainer,trainerId:leftTrainerId,setTrainerId:setLeftTrainerId,pokemon:leftPokemon,pokemonId:leftPokemonId,setPokemonId:setLeftPokemonId,opponent:rightPokemon,opponentTrainerId:rightTrainerId}),
      e.jsx(Fighter,{side:"Fighter 2",sideKey:"right",trainer:rightTrainer,trainerId:rightTrainerId,setTrainerId:setRightTrainerId,pokemon:rightPokemon,pokemonId:rightPokemonId,setPokemonId:setRightPokemonId,opponent:leftPokemon,opponentTrainerId:leftTrainerId})
    ]}),
