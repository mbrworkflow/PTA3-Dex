...
function BattleMode(){
  ...
  const updateDamage=(trainerId,pokemonId,amount)=>updatePokemon(trainerId,pokemonId,pokemon=>{const max=calculatePokemonMaxHP(pokemon),next=Math.min(max,Math.max(0,(pokemon.currentDamage||0)+amount));return{...pokemon,currentDamage:next,battleAffliction:next>=max&&["burned","cursed","poisoned","toxified"].includes(pokemon.battleAffliction?.type)?null:pokemon.battleAffliction}});
